...
public class Connect {
  ...
  public String rest(String url, HttpMethod method, String token, Object body) {
    try {
      log.info("rest-connector::method::{}::url-details::{}::body-details::{}::auth-token::{}", method, url, body,
          token);
      ...
      retryTemplate.execute((RetryCallback<Boolean, Exception>) context -> {
        ...
        final Map<String, Object> responseDetails =
            objectMapperForRest.readValue(response.getBody(), new TypeReference<HashMap<String, Object>>() {
            });
      ...
      throw new GenericException(ValidationErrors.CONNECTIVITY_ERROR, ex.getLocalizedMessage(), ex.getCause());
    }
  }

  private static boolean checkStatus(Map<String, Object> responseDetails) {
    return Objects.nonNull(responseDetails) && responseDetails.containsKey("status") && responseDetails.get("status")
        .equals("success") && responseDetails.containsKey("data");
  }
